...
class TokenSafetyLookupTable {
  ...
  createMap() {
    ...
    let uniDefaultTokens = store.getState().lists.byUrl[DEFUND_LIST].current?.tokens
    let uniExtendedTokens = store.getState().lists.byUrl[DEFUND_LIST].current?.tokens
